...
# Unbuffer stdout so you can see the progress dots
...
# get details from LinkedIn
        connection_details = self.lnk.get_profile(member_id=id,selectors=['id','headline','first-name','last-name','summary','positions','siteStandardProfileRequest','pictureUrl','location'])

# linkedIn sometime gives us rubbish so check it
        if not connection_details.has_key('id'):
            return(False, connection_details)

        if connection_details['id']=="private":
            return(False, connection_details)
...
# if there is no mapping then add user unless on the blocked list
        if len(self.nomapping)==0:
            print "\nNo new contacts found."
        else:
            print "\nFound these contacts to add:"
            print self.nomapping
        for x in self.nomapping:
            if x not in self.exclude:
                print "\nAdding new contact: " + x.encode('ascii', 'ignore').decode('ascii')
                (rc, dets)=self.linkedIn.getDetails(x)
                if rc:
                    self.crm.addContact(dets, self.who)
                else:
                    print "###########\nERROR: LinkedIn not returning any valid data."
                    print json.dumps(dets, indent=3)
                    print "Skipped " + x.encode('ascii', 'ignore').decode('ascii')
            else:
                print ".",

# See if we are skipping the checks
        if check:
...
# If we have a match then check it is still right
            ...
            print "NOT Checking existing contacts."
...
if __name__ == "__main__":
    ...
    for key in sorted(i.lnk_connections):
        try:
            print key
        except UnicodeEncodeError:
            print "********* " + key.encode('ascii', 'ignore').decode('ascii')
    print i.getDetails("Brendan Clarke")
